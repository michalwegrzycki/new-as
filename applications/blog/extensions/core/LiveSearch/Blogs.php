	...
	public function getResults( $searchTerm )
	{
		...
		if( $this->hasAccess() )
		{
			...
			$blogs = \IPS\Db::i()->select(
							"*",
							'blog_blogs',
							array( "blog_club_id IS NULL AND ( (blog_member_id>? AND blog_name LIKE CONCAT( '%', ?, '%' ) ) OR ( word_custom LIKE CONCAT( '%', ?, '%' ) AND lang_id=? ) )", 0, $searchTerm, $searchTerm, \IPS\Member::loggedIn()->language()->id ),
							NULL,
							array( 0, 500 )
					)->join(
							'core_sys_lang_words',
