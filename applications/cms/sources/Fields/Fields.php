		...
		else if ( $type === 'thumbs' and $this->type === 'Upload' )
		{
			if ( isset( $this->extra['thumbsize'] ) )
			{
				...
				else
				{
					try
					{
						return \IPS\File::get( static::$uploadStorageExtension, \IPS\Db::i()->select( '*', 'cms_database_fields_thumbnails', array( array( 'thumb_field_id=?', $this->id ) ) )->setKeyField('thumb_original_location')->first() )->url;
					}
					catch( \Exception $e ) { }
				}
			}
