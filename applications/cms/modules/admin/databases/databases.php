						...
						if ( $xml->name == $data['next'] )
						{
							...
							switch ( $areaName )
							{
								case 'field':
									...
									if ( ! class_exists( '\IPS\cms\Fields\\' . ucfirst( $attrs['field_type'] ) ) and ! class_exists( '\IPS\Helpers\Form\\' . ucfirst( $attrs['field_type'] ) ) )
									{
	...
	protected function _getDatabaseForm( $current, $category )
	{
		...
		if ( \IPS\Application::appIsEnabled( 'forums' ) )
		{
			...
			if ( $current )
			{
				...
				\IPS\Member::loggedIn()->language()->words['database_forum_record_desc'] = \IPS\Member::loggedIn()->language()->addToStack( 'database_forum_record__desc' ) . ' ' . \IPS\Member::loggedIn()->language()->addToStack( 'database_forum_record__rebuild', NULL, array( 'sprintf' => array( $rebuildUrl ) ) ) . ' ' . \IPS\Member::loggedIn()->language()->addToStack( 'database_forum_comments__rebuild', NULL, array( 'sprintf' => array( $rebuildUrl ) ) );
			}
			else
			{
