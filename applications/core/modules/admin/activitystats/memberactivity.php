 ...
 * memberactivity
 */
class _memberactivity extends \IPS\Dispatcher\Controller
{
	/**
	 * Execute
	 *
	...
	{
		\IPS\Dispatcher::i()->checkAcpPermission( 'memberactivity_manage' );
		parent::execute();
	}
	...
	protected function manage()
	{
		$chart = new \IPS\Helpers\Chart\Callback( 
			\IPS\Http\Url::internal( 'app=core&module=activitystats&controller=memberactivity' ), 
		...
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('menu__core_activitystats_memberactivity');
	...
	protected function getSqlResults( $table, $date, $author, $chart )
	{
		...
		if ( !$chart->timezoneError and \IPS\Member::loggedIn()->timezone and in_array( \IPS\Member::loggedIn()->timezone, \DateTimeZone::listIdentifiers() ) )
		{
