...
class _devices extends \IPS\Dispatcher\Controller
{
			...
			$table->rowButtons = function( $row ) {
				return array(
					'view'	=> array(
						'title'	=> 'view',
						'icon'	=> 'search',
						'link'	=> \IPS\Http\Url::internal( 'app=core&module=members&controller=members&do=edit' )->setQueryString( 'id', $row['member_id'] ),
					),
				);
			};
	...
	{		
		try
		{
			$device = \IPS\Member\Device::loadAndAuthenticate( \IPS\Request::i()->device, \IPS\Member::load( \IPS\Request::i()->member ) );
			$device->login_key = NULL;
			$device->save();
			
			\IPS\Db::i()->delete( 'core_sessions', array( 'member_id=? AND browser=?', $device->member_id, $device->user_agent ) );
		}
		catch ( \Exception $e ) { } 
