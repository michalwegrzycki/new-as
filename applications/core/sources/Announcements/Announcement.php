...
class _Announcement extends \IPS\Content\Item
{
	...
	{
		parent::save();
		static::rebuildAnnouncementsExist( TRUE );
	}
	 ...
	 * @return	void
	 */
	protected static function rebuildAnnouncementsExist( $value = NULL )
	{
		if ( $value === NULL )
		{
			$value = (bool) \IPS\Db::i()->select( 'COUNT(*)', 'core_announcements', 'announce_active=1', NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
		}
				
		\IPS\Settings::i()->changeValues( array( 'announcements_exist' => $value ) );
	}
}