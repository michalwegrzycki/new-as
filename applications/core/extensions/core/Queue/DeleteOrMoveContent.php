...
class _DeleteOrMoveContent
{
	...
	public function getProgress( $data, $offset )
	{
		...
		if ( isset( $data['moveTo'] ) )
		{
			...
			$text = \IPS\Member::loggedIn()->language()->addToStack('backgroundQueue_move_content', FALSE, array( 'htmlsprintf' => array( "<a href='{$node->url()}' target='_blank'>{$node->_title}</a>", "<a href='{$moveTo->url()}' target='_blank'>" . strip_tags( $moveTo->_title ) . "</a>" ) ) );
		}
		else
		{
