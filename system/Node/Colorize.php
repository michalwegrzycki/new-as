...
trait Colorize
{
	...
	{
		if ( ! isset( $containerData[ static::$featureColumnName ] ) )
		{
		...
		$node['text_color'] = static::featureTextColor( $node[ static::$featureColumnName ] );
		
		/* Normalize */
		$node['feature_color'] = $node[ static::$featureColumnName ];
		...
		{
			return \IPS\Member::loggedIn()->language()->addToStack( 'club_container_title', FALSE, array( 'htmlsprintf' => array( $containerData['_club']['name'], $title ) ) );
		}
		else
		{
			return $title;
