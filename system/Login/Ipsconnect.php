...
class _Ipsconnect extends LoginAbstract
{
	...
	public function authenticate( $values )
	{
		try
		{
			...
			{
				if( isset( $response['status'] ) AND $response['status'] == 'ACCOUNT_NOT_FOUND' )
				{
	...
	protected function get__enabled()
	{
		...
		if( $result === FALSE )
		{
			if( \IPS\Db::i()->select( 'COUNT(*)', 'core_ipsconnect_slaves', array( 'slave_enabled=?', 1 ) )->first() > 0 )
			{
				return NULL;
			}
		}
	...
	public function onDisable()
	{
		...
		{
			$response = \IPS\Http\Url::external( $this->settings['url'] )
				->setQueryString( array( 'do' => 'disableSlave', 'key' => $this->settings['key'], 'url' => \IPS\Settings::i()->base_url . '/applications/core/interface/ipsconnect/ipsconnect.php' ) )
				->request()
				->get()
				->decodeJson();
		}
