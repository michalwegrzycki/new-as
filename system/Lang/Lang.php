...
class _Lang extends \IPS\Node\Model
{
	...
	public function preferredDateFormat()
	{
		...
		return str_replace( array( '1992', '92', '03', '3', $date->strFormat('%B'), $date->strFormat('%b'), '04', ' 4', '4' ), array( 'YY', 'YY', 'MM', 'MM', 'MM', 'MM', 'DD', 'DD', 'DD' ), $date->localeDate() );
	}
	...
	{
		$i = 0;
		$openCurly  = '--' . uniqid( '', TRUE ) . '--';
		$closeCurly = '--' . uniqid( '', TRUE ) . '--';
	...
	public function formatList( $items, $format=NULL )
	{
		...
		{
			$items[ $i ] = preg_replace( '/(?!&)#(?!\d{2,4};)/', '\#', $items[ $i ] );
			$upper = FALSE;
			if ( $i == 0 )
			{
				$upper = ( $matches[3] === 'A' );
